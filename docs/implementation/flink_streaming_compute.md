...
|---|---|---|---|---|---|---|---|---|---|---|
| `P2-10.S1` | Shared | Lock Flink, connector, Iceberg, job, image, savepoint compatibility, and verifier artifacts. | Build owner | P2-8 developer gate | `platform/flink/image/`; jobs/locks | scan, digest, job startup and connector smoke | D1, P1-P3 | Platform owner | Connector compatibility | Not started |
| `P2-10.D1` | Developer | Deploy idempotent reduced Flink with local state and Kafka connectivity. | Data-engineering owner | `P2-10.S1` | `platform/flink/developer/` | lifecycle, TaskManager registration, checkpoint | D1 | Platform owner | Local resources | Not started |
...
| `P2-10.P1` | Production | Deploy HA JobManagers/TaskManagers, ZooKeeper namespace, placement, capacity, and restricted REST. | Platform owner | `P2-10.S1`, P2-8 production gate | `platform/flink/`; `environments/production/flink/` | leader failover, worker loss, capacity | P1-P8 | Operations owner | HA exception | Not started |
| `P2-10.P2` | Production | Configure Ceph checkpoints/savepoints/HA state, internal TLS/mTLS, managed secrets, and Kafka ACLs. | Security/data-platform owners | `P2-10.P1` | `platform/flink/config/`; `environments/production/flink/` | state read/write, TLS/auth negative tests, rotation | P5-P11 | Security owner | State compatibility | Not started |
| `P2-10.R1` | Production | Execute JM/TM/Kafka/Ceph outage, restore, upgrade/rollback, alert, and orphan-state cleanup drills. | Operations owner | `P2-10.P2` | `operations/runbooks/flink/` | timed drills, savepoint restore, alerts | P10-P14 | Platform owner | Maintenance window | Not started |
