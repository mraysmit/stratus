...
This section is the concise status view; the Implementation Task Track in
section 12 owns the task definitions, dependencies, evidence and acceptance
state. Completed developer telemetry is deliberately distinguished from the
production observability stack. A successful local transcript proves the
instrumentation and correlation contract, but it does not prove durable
collection, dashboards, alerts or production retention.

| Task | Status | Delivered or remaining outcome |
|---|---|---|
| `P1-3.1-V1` **Verified 2026-08-16** | Completed | One SLF4J 2.x provider; declarative INFO/DEBUG control; sanitized failure detail; MDC correlation across shell, JUnit, in-process jobs, packaged drivers and executors; suite/class/test/client/session/SQL/subprocess/job-phase durations; timing distributions; and per-SQL Spark scheduler/executor statistics. The authoritative 46-test transcript is `platform/spark/compose-cluster/logs/spark-conformance-tests-20260816T071313Z.log`. |
| `P1-3.1-V2` **Verified 2026-08-16** | Completed | Prepared-artifact focused runner, one suite-owned Spark context with isolated sessions, action batching, isolated quality-result tables, and test entry points under `scripts/tests/`. The authoritative run passed 46/46 with 198 visible scheduler jobs and 141.894 seconds of JUnit time; the relocated focused runner passed 5/5 in `spark-conformance-tests-20260816T111556Z.log`. |
| `P1-3.1-V3` **Not started** | Remaining | Add named measurements for Maven preparation versus JUnit, Spark context construction and master connection, first executor registration and first completed job, catalog initialization, Iceberg scan planning and commit, external submission process versus registered application lifetime, and fixture setup/cleanup. Collect stable samples, publish an advisory baseline, and introduce relative regression thresholds only after owner review. |
| `P1-3.1-V4` **Not started** | Remaining | Split and document selectable **Fast live**, **Deep semantic**, and **Cold packaged submission** tiers. Prove that the authoritative release composition includes every tier and that focused developer execution cannot be mistaken for release evidence. |
| `P1-3.6-P2` **Not started** | Remaining production work | Export Spark service, application and job telemetry into the platform Prometheus, Grafana and Loki stack; provide correlated dashboards, queries and alerts; prove bounded retention and continuity; and preserve `suiteRunId`, `jobRunId` and `operationId` across log ingestion. OpenTelemetry may be adopted only through the platform's approved telemetry architecture rather than as a private Spark-only path. |
...
| `P1-3.1-V4` | `P1-3.1` | Verification | Separate fast live, deep semantic and cold packaged-submission feedback tiers; done when each tier is independently selectable, documented, non-overlapping where intended, and the authoritative release command proves their union. | QA owner | `P1-3.1-V2` | Maven/JUnit selectors; test scripts; operations documentation | selector guardrails and one passing transcript per tier plus aggregate evidence | P10-P13 | Platform owner | A focused or fast run must never be presented as complete release evidence. | Not started |
...
| `P1-3.6-P2` | `P1-3.6` | Production | Integrate Spark logs and metrics with the platform observability stack; done when Prometheus collection, Grafana dashboards and Loki ingestion expose correlated service/application/job health with reviewed alerts and retention. | Operations owner | `P1-3.6-P1`, Phase 1 observability stack | production Spark monitoring configuration; dashboards; alerts; runbook | scrape/ingestion health, correlation queries, alert drill and retention evidence | P12, Phase 1 readiness | Platform owner | Log-only developer telemetry is not production monitoring; an OpenTelemetry path requires an approved cross-platform decision. | Not started |
