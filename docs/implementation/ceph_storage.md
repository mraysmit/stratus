...
- Status: Developer track complete - accepted by the platform owner on 2026-08-03 with two recorded deferrals (immutable verifier-image publication under `P1-0.1`, and quantitative performance thresholds carried into `P1-1.6-P1`); production track not started
...
| `P1-1.6-D1` | `P1-1.6` | Developer | Run the Java storage conformance suite from the published verifier image. Done when all bucket, object, multipart, TLS, and access-boundary tests pass. | Quality engineering owner | `P1-1.2-D3` | `verification/storage/`; `platform/ceph/compose-cluster/evidence/` | Image reference, coverage report (reported for visibility, never gated), twelve-check live report, invalid-auth/policy/TLS reports, persistent debug log | D10, D12 | Data platform owner | Offline suite and repository guardrail tests pass; live object, pagination, concurrency, multipart, invalid-auth, cross-identity, and untrusted-TLS checks pass from the developer-built image, most recently 2026-08-03 alongside the 27-test live JVM conformance suite (SDK, raw REST with the business dataset lifecycle, Admin Ops, Dashboard). Immutable registry publication remains deferred under `P1-0.1` by owner direction, reaffirmed 2026-08-03 | Accepted |
...
- [ ] **D2** - the build pipeline publishes the storage verifier image by immutable digest with scan and provenance evidence — *deferred under `P1-0.1` by owner direction (2026-07-14, reaffirmed 2026-08-03); carried into the production track*
...
- [ ] **D12** - verifier execution uses the published image, protected configuration injection, read-only trust material, and dedicated evidence mount without source or build tools — *all clauses except the published image are proven; the published-image clause shares the `P1-0.1` deferral with D2*
- [x] **D13** - Ceph health checks show expected state
- [x] **D14** - lab failure drill behavior is documented

The developer gate was accepted by the platform owner on 2026-08-03 with the
two `P1-0.1` publication deferrals recorded above; every other gate is
evidenced by the 2026-08-03 full-sequence transcripts (see `P1-1.G-D`).
