...
- Status: Development stage complete - accepted by the platform owner on 2026-08-03. The local verifier image, functional behavior, and development baselines are accepted; immutable publication and quantitative production thresholds belong to the later production deployment-hardening stage.
...
For Windows workstations, run the scripts from Git for Windows Bash (harness scripts ship as a single bash implementation per ADR-P1-002; PowerShell users invoke them as `bash scripts/lifecycle/ceph-compose-startup.sh`). Git Bash runs directly against Docker Desktop; a user Ubuntu WSL distribution does not need to be running. The Bash scripts centralize MSYS path handling so Windows host paths reach `docker.exe` in Windows form while container paths such as `/certs` and `/evidence` remain Linux paths. Do not reconstruct the underlying Compose invocation outside the supplied scripts. The repository is not mounted into the verifier container; developers build the pinned verifier image through the supplied build script before running this harness. The later production stage publishes that accepted image through the approved artifact pipeline.

The complete lifecycle was validated on 2026-07-20 with Docker Desktop (at that date the harness still shipped PowerShell twins; ADR-P1-002 retired them on 2026-07-22 and the bash scripts are now the only implementation): cluster startup, bucket bootstrap and smoke checks, all twelve Java S3 conformance checks, three security-negative checks, RGW/MON/OSD failure drills and recovery, shutdown, destructive reset, and the harness self-test. This evidence does not qualify Podman; Podman remains a separate runtime compatibility path.

On 2026-07-22, after the local CA and private keys were deliberately destroyed, the harness was re-validated end to end from Git Bash under the single-bash script implementation (ADR-P1-002): startup regenerated the `.env` credentials and the full certificate chain from scratch, all twelve containers reached healthy, bucket bootstrap and smoke checks passed, the Java conformance suite passed all twelve checks against the new chain, and the three security negatives (invalid credentials, cross-identity denial, untrusted TLS) failed as required. Transcript: `platform/ceph/compose-cluster/logs/ceph-local-verification-20260722-151214.txt`; evidence files stamped `20260722T0714Z` under `platform/ceph/compose-cluster/evidence/`. A regenerated CA must be re-imported wherever the previous CA was trusted (for example a desktop S3 client or OS trust store).

On 2026-07-30 the harness self-test, destructive reset, and a fresh startup-to-evidence cycle were re-validated; evidence files stamped `20260730T093638Z` and later are under `platform/ceph/compose-cluster/evidence/`.

On 2026-08-03 the full documented verification sequence was executed twice against the live cluster, the second time as a single command through the new `scripts/verify/ceph-compose-validate-cluster.sh` orchestrator (nine steps, `RESULT PASS`, six minutes end to end). Both runs included the live workstation JVM conformance suite: 27 tests across `CephRgwConformanceTest` (SDK S3 conformance), `CephS3RestConformanceTest` (raw Signature V4 REST conformance test including the versioned customer-master business dataset lifecycle: write, byte-identical read-back, rewrite with changed ETag, read-back, prefix listing, delete, and post-delete absence), `CephAdminOpsRestConformanceTest`, and `CephDashboardRestConformanceTest` — all passing with zero failures. Transcripts with explicit run boundaries are retained in `platform/ceph/tests/logs/` (`ceph-live-tests-20260803T*.log`, `rest-api-tests-20260803T*.log`, per-run `storage-verifier-*.log`) and `platform/ceph/compose-cluster/logs/validate-cluster-20260803T054452Z.txt`; harness evidence JSON is stamped `20260803T*` under `platform/ceph/compose-cluster/evidence/`.
...
The normative variable contract is the committed file `platform/ceph/compose-cluster/.env.template`; this runbook deliberately does not duplicate it. The first startup creates the ignored `.env` from that template and replaces the credential placeholders with generated per-machine disposable secrets.
...
- `scripts/verify/ceph-compose-validate-cluster.sh` — one-command comprehensive validation running the whole sequence above in order with a per-step transcript; `--full` wraps it in startup and shutdown
...
- S3 and Java conformance checks continue with one RGW stopped
- S3 and Java conformance checks continue during one-OSD degradation, followed by recovery to `HEALTH_OK` and all placement groups `active+clean`
- S3 client path-style access works
...
The required developer topology is Docker Desktop, Docker Engine, or Podman running `platform/ceph/compose-cluster/compose.yaml`. It deploys three genuine Ceph MONs, two MGRs, three BlueStore OSDs, two RGWs, the TLS proxy, rclone, and the prebuilt Stratus verifier image. A shared acceptance environment may provide additional evidence but does not replace the disposable Compose topology.
...
The verification suite proves the Stratus S3 storage conformance against Ceph RGW.
...
- A task moves to `Verified` only when its deliverable exists and the listed evidence is stored in the owning component's evidence directories (`platform/ceph/compose-cluster/evidence/` and `platform/ceph/tests/logs/`, indexed by run timestamp) or the approved evidence system using the task ID.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `P1-1.1-S1` | `P1-1.1` | Shared | Approve Ceph RGW as the baseline, selected release, supported host OS/runtime combinations, production topology, and reconsideration triggers. Done when the decision record is accepted. | Platform architect | Architecture requirements; no production publication prerequisite for development acceptance | `docs/decisions/ADR-P1-001-ceph-baseline.md`; version matrix entry | Accepted ADR, upstream compatibility references, image/package checksums | D1, P1, P2 | Architecture owner | Production support and topology decisions remain for the later production deployment-hardening stage; Ceph 20.2.2 image digests are recorded | Accepted |
| `P1-1.2-S1` | `P1-1.2` | Shared | Define node, disk, network, DNS, NTP, firewall, capacity, CRUSH failure-domain, pool, and endpoint requirements. Done when values are reviewable and contain no unresolved placeholder used for deployment. | Storage owner | `P1-1.1-S1` | `platform/ceph/cephadm-cluster/`; `environments/<environment>/ceph/`; capacity worksheet | Peer-reviewed inventory/specs, address and port validation, raw-to-usable calculation | D7, P1-P4 | Operations owner | Hardware and addresses may be environment-specific | Not started |
| `P1-1.6-S1` | `P1-1.6` | Shared | Implement and lock the storage verifier artifact, local image, and Java suite. Done for development when the pinned local image passes offline tests, container smoke, and live conformance. | Build engineering owner | `P1-1.1-S1` | `verification/storage/src/test/`; `verification/storage/image/`; local image build and test scripts | Unit report, pinned-input record, container smoke test, live conformance report | D2, D10-D12 | Platform owner | Immutable registry publication, scan, SBOM, provenance, and signing are later production deployment-hardening work under `P1-0.1` | Accepted |
| `P1-1.2-D1` | `P1-1.2` | Developer | Select and record the required disposable Compose Ceph profile. Done when prerequisites and unsupported production claims are recorded. | Developer-platform owner | `P1-1.1-S1` | `platform/ceph/compose-cluster/.env.template`; `docs/decisions/ADR-P1-001-ceph-baseline.md` | Compose validation, pinned image inspection, runtime profile and limitation review | D3 | Platform owner | Container-level quorum and recovery do not establish physical-host, rack, site, capacity, or production durability claims | Accepted |
| `P1-1.3-D1` | `P1-1.3` | Developer | Generate the developer CA chain and RGW certificate without copying private keys into client/verifier containers. Done when hostname validation succeeds. | Security engineering owner | `P1-1.2-D1` | ignored `platform/ceph/compose-cluster/certs/` and `private/` paths | `openssl verify`, endpoint handshake, SAN validation, file-permission evidence | D5 | Security owner | Certificate chain verification passed; regeneration from destroyed keys re-verified 2026-07-22; security acceptance recorded with the developer gate 2026-08-03 | Accepted |
| `P1-1.2-D2` | `P1-1.2` | Developer | Deploy the disposable Compose Ceph endpoint. Done when three MONs, two MGRs, three BlueStore OSDs, and two RGWs run from the pinned image and the single-infrastructure-host limitations are documented. | Storage owner | `P1-1.2-D1` | `platform/ceph/compose-cluster/compose.yaml`; `platform/ceph/compose-cluster/ceph/` | `ceph status`, quorum and manager inventory, OSD tree and pool detail, RGW S3 round trip, runtime/version output | D4, D7, D13 | Operations owner | Verified `HEALTH_OK`, 3/3 MON quorum, active/standby MGR, 3 OSDs `up`/`in`, two RGWs, and 321 placement groups `active+clean`; developer acceptance recorded 2026-08-03; production acceptance belongs to the production track | Accepted |
| `P1-1.3-D2` | `P1-1.3` | Developer | Configure the Compose-cluster RGW HTTPS proxy and apply the developer certificate. Done when trusted clients connect and plaintext/insecure paths are rejected. | Storage owner | `P1-1.3-D1`, `P1-1.2-D2` | `platform/ceph/compose-cluster/ceph/nginx.conf`; Compose TLS mounts | TLS handshake, CA validation, HTTPS S3 list, negative HTTP or untrusted-CA test | D5 | Security owner | Trusted HTTPS passed; untrusted CA was rejected and plaintext RGW was not published; security acceptance recorded with the developer gate 2026-08-03 | Accepted |
| `P1-1.4-D1` | `P1-1.4` | Developer | Create the five buckets and acceptance-equivalent scoped identities. Done when positive and negative access tests match the policy matrix. | Storage owner | `P1-1.3-D2` | developer RGW users plus `platform/ceph/compose-cluster/scripts/verify/ceph-compose-bootstrap-buckets.*` and `ceph-compose-verify-security.*` | Bucket/user inventory plus invalid-credential and cross-identity denial reports | D6, D8, D9 | Security owner | Positive operations, invalid credentials, and cross-identity HTTP 403 passed in the developer environment, re-validated 2026-07-22 with regenerated credentials and 2026-08-03 in both full-sequence runs; evidence is indexed by run timestamp in the component evidence directories; security acceptance recorded with the developer gate 2026-08-03 | Accepted |
| `P1-1.2-D3` | `P1-1.2` | Developer | Implement the idempotent Compose Ceph and client/verifier harness. Done when startup, check, verifier, shutdown, reset, and repeated startup succeed without manual repair. | Developer-platform owner | `P1-1.3-D1`, `P1-1.4-D1`, `P1-1.6-S1` | `platform/ceph/compose-cluster/compose.yaml`; `.env.template`; lifecycle scripts | Compose validation, real Ceph daemon health, HTTPS S3 round trip, clean reset/start transcript, no Compose build configuration | D3, D12 | Platform owner | Empty-volume recreation, rclone, and the externally prebuilt Java verifier passed; reset/shutdown also remove profiled verifier containers; full lifecycle re-validated 2026-07-22 under the single-bash implementation (ADR-P1-002) and 2026-08-03 as one command via `ceph-compose-validate-cluster.sh --full`; formal image publication remains outside this task | Accepted |
| `P1-1.6-D1` | `P1-1.6` | Developer | Run the Java storage conformance suite from the developer-built verifier image. Done when all bucket, object, multipart, TLS, and access-boundary tests pass. | Quality engineering owner | `P1-1.2-D3` | `verification/storage/`; `platform/ceph/compose-cluster/evidence/` | Local image identity, coverage report (reported for visibility, never gated), twelve-check live report, invalid-auth/policy/TLS reports, persistent debug log | D10, D12 | Data platform owner | Offline suite and repository guardrail tests pass; live object, pagination, concurrency, multipart, invalid-auth, cross-identity, and untrusted-TLS checks pass from the developer-built image, most recently 2026-08-03 alongside the 27-test live JVM conformance suite (SDK, raw REST with the business dataset lifecycle, Admin Ops, Dashboard). Immutable registry publication belongs to the later production deployment-hardening stage under `P1-0.1` | Accepted |
| `P1-1.6-D2` | `P1-1.6` | Developer | Run storage-only concurrency, multipart, small-object, and prefix-listing baselines. Done when declared thresholds pass or an owned decision record accepts a variance. | Performance engineering owner | `P1-1.6-D1` | `verification/storage/`; benchmark result bundle | Raw runs, p50/p95/p99, throughput, errors/retries, object counts, environment manifest | D11 | Platform owner | Functional eight-way concurrency, forced pagination, multipart, throttling retry, and bounded timeout checks pass. Per the definition of done, the owner accepted a variance on 2026-08-03: quantitative latency/throughput/object-count thresholds and benchmark evidence are carried forward into the production qualification task `P1-1.6-P1` | Accepted |
| `P1-1.5-D1` | `P1-1.5` | Developer | Exercise health observation and the supported local MON, RGW, and OSD failure/recovery paths. Done when degradation, client continuity, and recovery match the documented behavior. | Operations owner | `P1-1.2-D2`, `P1-1.6-D1` | Local drill record and health snapshots | Two-of-three quorum, RGW failover client checks, before/during/after OSD `ceph status`, client read/write, final clean PG state | D13, D14 | Operations owner | Verified two-MON quorum, rclone/Java success with one RGW and one OSD offline, and recovery to `HEALTH_OK`; single-host profile cannot prove physical-host failure; developer acceptance recorded 2026-08-03 | Accepted |
| `P1-1.G-D` | `P1-1` | Developer | Review and accept the developer gate. Done only when D1-D14 have producing accepted tasks and no development evidence is missing. | Platform owner | `P1-1.6-D2`, `P1-1.5-D1` | Developer gate record | Gate-to-task matrix, evidence index, open-risk review | D1-D14 | Platform and data-platform owners | Accepted by the platform owner 2026-08-03 with every development gate evidenced. Immutable registry publication and quantitative production qualification are mapped to the later production deployment-hardening stage and are not development-gate conditions. Evidence: 2026-08-03 transcripts in `platform/ceph/tests/logs/` and `platform/ceph/compose-cluster/logs/validate-cluster-20260803T054452Z.txt` | Accepted |
...
- [x] **D1** - approved Ceph release is selected and pinned
- [x] **D2** - the storage verifier image is built locally from pinned inputs, its identity is recorded, and its smoke and conformance checks pass; immutable publication evidence belongs to the later production deployment-hardening stage
- [x] **D3** - the disposable genuine Ceph/RGW local Docker runtime profile and its unsupported production claims are recorded
- [x] **D4** - Ceph lab cluster is running
- [x] **D5** - RGW is reachable over HTTPS
- [x] **D6** - all five Stratus S3 buckets exist
- [x] **D7** - pool and CRUSH layout are documented
- [x] **D8** - platform service identities or lab equivalents exist
- [x] **D9** - access boundaries are enforced through RGW policy/user controls
- [x] **D10** - Java S3 verification suite passes
- [x] **D11** - concurrent synthetic S3, multipart, and small-object/prefix-listing baselines pass without later engines
- [x] **D12** - verifier execution uses the already-built local development image, protected configuration injection, read-only trust material, and a dedicated evidence mount without source or build tools
- [x] **D13** - Ceph health checks show expected state
- [x] **D14** - lab failure drill behavior is documented

The developer gate was accepted by the platform owner on 2026-08-03 with all
D1-D14 development conditions evidenced by the 2026-08-03 full-sequence
transcripts (see `P1-1.G-D`). Immutable publication and production performance
qualification are separate later-stage conditions, not exceptions to this gate.
Increment 2 engineering work can begin.
