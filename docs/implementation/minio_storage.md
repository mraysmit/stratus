...
The storage architecture decision, candidate comparison, scoring, and proof-of-fit gate are owned by [stratus_on_prem_data_fabric_architecture.md](../architecture/stratus_on_prem_data_fabric_architecture.md#28-storage-architecture-decision). This document is a legacy implementation variant for a MinIO/AIStor path and applies only if a future architecture decision explicitly selects that target.
