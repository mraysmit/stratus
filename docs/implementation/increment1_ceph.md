...
services:
  ...
  verifier:
    ...
    command: >
      ...
      -storepass changeit
      -noprompt &&
      exec java -jar /opt/stratus/storage-contract-verifier.jar
    environment:
...
```text
verification/storage-contract/src/test/java/dev/stratus/verification/storage/CephRgwIntegrationTest.java
```
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `P1-1.1-S1` | `P1-1.1` | Shared | Approve Ceph RGW as the baseline, selected release, supported host OS/runtime combinations, production topology, and reconsideration triggers. Done when the decision record is accepted. | Platform architect | `P1-0.1` | `docs/decisions/ADR-P1-001-ceph-baseline.md`; version matrix entry | Accepted ADR, upstream compatibility references, image/package checksums | D1, P1, P2 | Architecture owner | None recorded | Not started |
| `P1-1.2-S1` | `P1-1.2` | Shared | Define node, disk, network, DNS, NTP, firewall, capacity, CRUSH failure-domain, pool, and endpoint requirements. Done when values are reviewable and contain no unresolved placeholder used for deployment. | Storage owner | `P1-1.1-S1` | `platform/ceph/cephadm/`; `environments/<environment>/ceph/`; capacity worksheet | Peer-reviewed inventory/specs, address and port validation, raw-to-usable calculation | D7, P1-P4 | Operations owner | Hardware and addresses may be environment-specific | Not started |
| `P1-1.6-S1` | `P1-1.6` | Shared | Implement and publish the storage verifier image and Java suite. Done when CI publishes a scanned immutable digest with provenance and a smoke report. | Build engineering owner | `P1-0.1`, `P1-1.1-S1` | `verification/storage-contract/src/test/`; `verification/storage-contract/image/`; CI pipeline definition | Unit report, image scan, SBOM, provenance, digest, container smoke test | D2, D10-D12, P10 | Platform owner | Local artifact and image are built; registry publication, scan, SBOM, and signing remain | Built |
| `P1-1.2-D1` | `P1-1.2` | Developer | Select and record the developer profile: Docker Desktop client harness, Podman client harness, single-host cephadm, or representative lab. Done when prerequisites and unsupported production claims are recorded. | Developer-platform owner | `P1-1.1-S1` | `environments/developer/ceph/profile.env.template`; implementation record | Preflight output for runtime, DNS, ports, endpoint reachability, and writable evidence directory | D3 | Platform owner | Shared RGW lab may be unavailable | Not started |
| `P1-1.3-D1` | `P1-1.3` | Developer | Generate or obtain the developer CA chain and RGW certificate without copying private keys into the client harness. Done when hostname validation succeeds. | Security engineering owner | `P1-1.2-D1` | `platform/ceph/developer/certs/stratus-ca.crt`; protected lab PKI location | `openssl verify`, endpoint handshake, SAN validation, file-permission evidence | D5 | Security owner | Certificate helpers are built; OpenSSL and a selected RGW endpoint are required for verification | Built |
| `P1-1.2-D2` | `P1-1.2` | Developer | Deploy or select the Ceph lab endpoint. Done when cephadm reports the expected lab daemons healthy and the profile limitations are documented. | Storage owner | `P1-1.2-S1`, `P1-1.2-D1` | `platform/ceph/cephadm/`; `environments/lab/ceph/`; lab deployment record | `ceph status`, `ceph orch ps`, OSD/pool inventory, runtime/version output | D4, D7, D13 | Operations owner | Docker Desktop cannot host the Ceph cluster | Not started |
...
| `P1-1.4-D1` | `P1-1.4` | Developer | Create the five buckets and lab-equivalent scoped identities. Done when positive and negative access tests match the policy matrix. | Storage owner | `P1-1.3-D2` | `platform/ceph/bootstrap/`; `environments/developer/ceph/`; protected credential records | Bucket inventory, RGW user inventory, allowed/denied operation report | D6, D8, D9 | Security owner | Credentials must not enter Git or evidence output | Not started |
| `P1-1.2-D3` | `P1-1.2` | Developer | Implement the idempotent Docker Desktop and Podman client harness. Done when startup, check, verifier, shutdown, and repeated startup succeed without manual repair. | Developer-platform owner | `P1-1.3-D1`, `P1-1.4-D1`, `P1-1.6-S1` | `platform/ceph/developer/compose.yaml`; `.env.template`; `scripts/startup.sh`; `check.sh`; `verify-java.sh`; `shutdown.sh` | Shell/static checks, Docker Compose run, Podman run, second-start and second-stop transcripts | D3, D12 | Platform owner | Docker lifecycle passed; live check/verifier and Podman lifecycle remain | Built |
| `P1-1.6-D1` | `P1-1.6` | Developer | Run the Java storage contract suite from the published verifier image. Done when all bucket, object, multipart, TLS, and access-boundary tests pass. | Quality engineering owner | `P1-1.2-D3` | Test report under the task evidence directory | Immutable image digest, injected-config record, JUnit report, no-source/no-build-tools inspection | D10, D12 | Data platform owner | Unit and container checks pass; scoped live Ceph endpoint and credentials are required | Built |
