...
- [ ] **D2** - In-memory persistence, local credentials, local CA material, and reduced topology are labelled developer-only in the promotion manifest. The plain-HTTP loopback this clause also named was removed on 2026-08-08 rather than labelled: TLS now terminates at the harness proxy and no plain-HTTP port is served.
