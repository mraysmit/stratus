...
| `P1-2.G-D` | `P1-2` | Developer | Accept developer gate after D1-D2 have accepted producing tasks. | Platform owner | `P1-2.4-V1` | developer gate record | Gate matrix and evidence index | D1-D2 | Data-platform owner | Accepted 2026-08-09 by the platform owner. All four producing tasks are `Accepted`, the gate matrix and evidence index in §13 resolve, and no functional item is outstanding — the TLS clause was closed on 2026-08-08 rather than deferred. Developer-only conditions remain recorded in the promotion manifest and are replaced on the production track | Accepted |
...
| Lab CA material for the catalog endpoint: TLS terminates at this harness's own nginx proxy for `polaris.stratus.local`, signed by a disposable CA the harness mints itself (closed 2026-08-08; plain HTTP is no longer served or configurable) | `P1-7.4` replaces the certificate with FreeIPA Dogtag-issued material; `P1-2.2-P1` carries the production topology | never fall back to plain HTTP or relax client verification to make a TLS check pass — the workstation route was moved inside the harness network rather than weakened when Schannel refused the private CA |
...
- [x] **D1** - Disposable in-memory persistence mode starts/stops idempotently and the namespace, table, Iceberg metadata, Ceph RGW, and verifier conformance checks pass.
- [x] **D2** - In-memory persistence, local credentials, local CA material, and reduced topology are labelled developer-only in the promotion manifest. The plain-HTTP loopback this clause also named was removed on 2026-08-08 rather than labelled: TLS now terminates at the harness proxy and no plain-HTTP port is served.
...
**Accepted 2026-08-09.** The platform owner accepted the Increment 2 developer gate
on this evidence: `P1-2.2-D1`, `P1-2.3-D1`, `P1-2.4-V1`, and `P1-2.5-D1` are
all `Accepted`, satisfying the gate traceability rule, and D1 and D2 are
ticked above. The live catalog conformance suite stood at 24 checks, all
passing, re-verified after a full secret rotation and again on a platform
rebuilt from destroyed volumes.

This accepts the **developer** track only. Increment 2 as a whole is not
accepted: the production metadata store, TLS for shared use, backup and
restore, and audit evidence remain open on the production track, and the
portfolio row in the Phase 1 plan stays `Not started` until they close.

The paragraph below records what was outstanding before acceptance and is kept
as the history of that decision.

**What stood between that evidence and a tick.** Per the gate traceability rule the four producing tasks must reach `Accepted`, which is the platform owner's action under `P1-2.G-D` and is deliberately not taken on the owner's behalf. **That is now the only thing outstanding.** The TLS clause was closed on 2026-08-08 rather than deferred: `polaris.stratus.local` is served by this harness's own nginx proxy over TLS, Polaris publishes no host port, and the plain-HTTP switch was deleted rather than set to false, so there is nothing left for an operator to turn back on. The certificate is signed by a disposable CA this harness mints, because the Ceph CA's signing key never crosses the harness boundary (ADR-P1-003); replacing that lab CA with FreeIPA Dogtag material remains with `P1-7.4`, and is recorded in the promotion manifest as a developer-only condition.
