...
**Prerequisites:**
- Increment 1 complete — Ceph RGW running, all buckets and service accounts in place
...
```bash
aws s3 --endpoint-url https://object-store.stratus.local ls --recursive s3://stratus-bronze/ | head -20
aws s3 --endpoint-url https://object-store.stratus.local ls --recursive s3://stratus-silver/ | head -20
aws s3 --endpoint-url https://object-store.stratus.local ls --recursive s3://stratus-gold/   | head -20
```
...
- [ ] Spark connects to Ceph RGW via S3A and can read and write all platform buckets
...
- Confirm the Ceph RGW endpoint is correct: `https://object-store.stratus.local`
- Test Ceph RGW access from a Spark node directly: `curl -k https://object-store.stratus.local/stratus-bronze`
